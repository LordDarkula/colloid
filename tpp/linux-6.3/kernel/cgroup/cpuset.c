...
// equivalent to normal cpuset_node_allowed
// only checks interrupts and if node is in cgroup membership list
int __cpuset_node_allowed_part_1(int node)
{
	if (in_interrupt())
		return 1;
	if (node_isset(node, current->mems_allowed))
...
bool __cpuset_node_allowed_part_2(int node, gfp_t gfp_mask)
{
	 ...
	 * been OOM killed to get memory anywhere.
	 */
	// unlikely is a branch-prediction hint for better performance.
	if (unlikely(tsk_is_oom_victim(current)))
		return true;
	// __GFP_HARDWALL means the kind of memory requested is strict about NUMA control
	if (gfp_mask & __GFP_HARDWALL)	/* If hardwall request, stop here */
		return false;
