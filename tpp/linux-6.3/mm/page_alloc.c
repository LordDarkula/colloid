...
retry:
	...
	for_next_zone_zonelist_nodemask(zone, z, ac->highest_zoneidx,
		...
		if (cpusets_enabled() && (alloc_flags & ALLOC_CPUSET)) {
			r = cpuset_zone_allowed_part_1(zone);
			if (r == 1 || (r == 2 && __memcg_zone_allowed(zone, order)))
				goto try_this_node;
			if (!cpuset_zone_allowed_part_2(zone, gfp_mask))
				continue;
		}
		...
		unsigned int *alloc_flags)
{
	...
	if (cpusets_enabled()) {
		*alloc_gfp |= __GFP_HARDWALL;
		 ...
		 */
		// create custome nodemask with per-node memory limits instead of just allowed cpu nodes for a cgroup
